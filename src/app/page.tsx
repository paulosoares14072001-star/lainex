      ...
      <div className="relative mx-auto flex min-h-screen max-w-7xl gap-6 p-6">
        <aside className="hidden w-72 rounded-3xl border border-white/10 bg-white/5 p-6 lg:block">
          <div className="mb-10">
            ...
            <p className="text-sm uppercase tracking-[0.35em] text-cyan-200">Control</p>
          </div>

          <nav className="space-y-2 text-sm">
            {["Dashboard", "Alunos", "Planos", "Financeiro", "Treinos", "Nutrição", "Atendimentos", "Equipe"].map((item, index) => (
              <div key={item} className={`rounded-2xl px-4 py-3 ${index === 0 ? "bg-gradient-to-r from-cyan-400/25 to-fuchsia-500/25 ring-1 ring-cyan-300/40" : "text-slate-300"}`}>
                {item}
              </div>
            ))}
          </nav>
        ...
        <section className="flex-1">
          <header className="mb-6 rounded-3xl border border-white/10 bg-white/5 p-8">
            <p className="text-sm uppercase tracking-[0.4em] text-cyan-200">Consultoria Online</p>
            <h2 className="mt-4 text-5xl font-black">Lainex Control</h2>
            <p className="mt-4 max-w-2xl text-slate-300">
              Painel central para controlar alunos, planos, financeiro, treinos, nutrição, atendimentos e equipe.
            </p>
          </header>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {[
              ...
              <div key={titulo} className="rounded-3xl border border-white/10 bg-white/5 p-6">
          ...
          <div className="mt-6 rounded-3xl border border-white/10 bg-white/5 p-6">
            ...
            <p className="mt-2 text-slate-300">
              Aqui vamos evoluir para cadastro de alunos, planos, pagamentos, treinos, dietas e relatórios.
            </p>
          </div>
        </section>
      </div>
